...
def cadastrar_empresa(request):
    ...
    if request.method == "GET":
        ...
        if not nome or not cnpj or not site:
            messages.add_message(request, constants.ERROR, 'Por favor, preencha todos os campos obrigatórios.')
            return redirect('/empresarios/cadastrar_empresa')

        

        if float(percentual_equity) <= 0 or float(percentual_equity) > 100:
            messages.add_message(request, constants.ERROR, 'O percentual de equity deve ser entre 0 e 100.')
            return redirect('/empresarios/cadastrar_empresa')
...
def empresa(request, id):
    ...
    if empresa.user != request.user:
        messages.add_message(request, constants.ERROR, "Essa empresa nao e sua.")
        return redirect(f'/empresarios/empresa/')
    
    if request.method == "GET":
        documentos = Documento.objects.filter(empresa=empresa)
        propostas_investimento = PropostaInvestimento.objects.filter(empresa=empresa)
        percentual_vendido = 0
        total_captado= 0
        
        
        
        for pi in propostas_investimento:
        
            if pi.status == 'PA':
                percentual_vendido = percentual_vendido + pi.percentual
                total_captado = sum(propostas_investimento.filter(status='PA').values_list('valor', flat=True))
                valuation_atual = (100 * float(total_captado)) / float(percentual_vendido) if percentual_vendido != 0 else 0
       
        valuation_atual = (100 * float(total_captado)) / float(percentual_vendido) if percentual_vendido != 0 else 0
        proposta_investimentos_enviada = PropostaInvestimento.filter(status='PE')
        return render(request, 'empresa.html', {'empresa': empresa, 'documentos': documentos, 'propostas_investimentos_enviada': proposta_investimentos_enviada, 'percentual_vendido': int(percentual_vendido), 'total_captado': total_captado, 'valuation_atual': valuation_atual })
